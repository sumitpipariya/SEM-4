...
EXEC PR_EMPLOYEE_DETAILS  NULL, 'Doe'
...
CREATE OR ALTER PROCEDURE PR_DEPT_DETAILS
...
BEGIN
	SELECT Projects.* 
	FROM Departments JOIN Projects
	ON Departments.DepartmentID = Projects.DepartmentID
	WHERE Projects.ProjectName = @ProjectName AND
	Departments.DepartmentName = @DepartmentName

END;

EXEC PR_PROJECT_DETAILS 'HR','Project Beta'
...
EXEC PR_EMPLOYEE_DATE '2010-06-15'
...
CREATE OR ALTER PROCEDURE PR_DETAILS_DEPT
	@Location VARCHAR(50)
AS
BEGIN
	SELECT * FROM Departments
	WHERE LEFT(Location,1) = @Location
END

EXEC PR_DETAILS_DEPT 'C'
...
EXEC PR_DEPT_DETAILS 'Project Delta','Chicago'